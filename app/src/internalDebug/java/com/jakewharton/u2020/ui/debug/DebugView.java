...
public final class DebugView extends FrameLayout {
    ...
    @BindView(R.id.debug_network_failure)
    Spinner networkFailureView;

    @BindView(R.id.debug_network_error)
    Spinner networkErrorView;

    @BindView(R.id.debug_network_error_code)
    Spinner networkErrorCodeView;
    ...
    @Inject
    @NetworkErrorPercent
    Preference<Integer> networkErrorPercent;

    @Inject
    Preference<NetworkErrorCode> networkErrorCode;

    @Inject
    MockResponseSupplier mockResponseSupplier;

    @Inject
    Application app;

    @Inject
    Set<DebugAction> debugActions;
    ...
    private <T extends Enum<T>> void configureResponseSpinner(Spinner spinner, final Class<T> responseClass) {
        ...
        RxAdapterView.itemSelections(spinner).map(adapter::getItem).filter(item -> item != mockResponseSupplier.get(responseClass)).subscribe(selected -> {
    ...
    private void setupUserInterfaceSection() {
        ...
        RxAdapterView.itemSelections(uiAnimationSpeedView).map(speedAdapter::getItem).filter(item -> !item.equals(animationSpeed.get())).subscribe(selected -> {
    ...
    private void showCustomEndpointDialog(final int originalSelection, String defaultUrl) {
        ...
        //
        new AlertDialog.Builder(getContext()).setTitle("Set Network Endpoint").setView(view).setNegativeButton("Cancel", (dialog, i) -> {
